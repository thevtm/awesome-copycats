...
local memcolor = "#777E76"

local mem = lain.widget.mem({
	settings = function()
		if mem_now.swapused >= 32 then
			widget:set_markup(markup.font(theme.font, string.format(" %3d%% %dM ", mem_now.perc, mem_now.swapused)))
		else
			widget:set_markup(markup.font(theme.font, string.format(" %3d%% ", mem_now.perc)))
		end
	end
})
--]]
...
local bat = lain.widget.bat({
	settings = function()
		if bat_now.status ~= "N/A" then
			...
			widget:set_markup(markup.font(theme.font, string.format(" %3d%% ", bat_now.perc)))
		else
			widget:set_markup()
			baticon:set_image(theme.widget_ac)
		end
...
function theme.at_screen_connect(s)
	...
	s.mywibox:setup {
		...
		{ -- Right widgets
			...
			arrow(theme.bg_normal, cpucolor),
			wibox.container.background(wibox.container.margin(wibox.widget { cpuicon, cpu.widget, layout = wibox.layout.align.horizontal }, 3, 4), cpucolor),
			
			arrow(cpucolor, memcolor),
			wibox.container.background(wibox.container.margin(wibox.widget { memicon, mem.widget, layout = wibox.layout.align.horizontal }, 2, 3), memcolor),
			
			--arrow(memcolor, "#4B696D"),
			--arrow("#4B696D", "#4B3B51"),
			...
			--arrow("#CB755B", batcolor),
			
			arrow(memcolor, batcolor),
			wibox.container.background(wibox.container.margin(wibox.widget { baticon, bat.widget, layout = wibox.layout.align.horizontal }, 3, 3), batcolor),
			
			--arrow(batcolor, "#C0C0A2"),
			...
			arrow(batcolor, volumecolor),
			wibox.container.background(wibox.container.margin(theme.volume.bar, 7, 7, 5, 5), volumecolor),

			arrow(volumecolor, binclockcolor),
			wibox.container.background(wibox.container.margin(binclock.widget, 4, 8), binclockcolor),
			
			arrow(binclockcolor, "alpha"),
			--]]
			s.mylayoutbox,
		},
	}
